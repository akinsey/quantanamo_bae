...
def backtest_trading_strategy(data, initial_capital, use_ai, model=None, scaler=None):
    ...
    for i in range(len(data) - 1):
        ...
        # Execute Sell Order
            ...
            if trade_signal == -1 or price_change >= MIN_PROFIT_THRESHOLD or price_change <= STOP_LOSS_THRESHOLD:
                ...
                trade_log.append(f"SELL {current_date.date()} @ {current_price:.2f} | Profit: {price_change * 100:.2f}%, Held: {days_held} days")
                position = 0
    ...
    print("\nTrade Summary:")
    print(" | ".join(trade_log[-10:]))  # Condensed trade output
    print(f"Final Portfolio Value: ${final_value:.2f}")
    print(f"Total Return: {total_return:.2f}% over {total_days} days")
