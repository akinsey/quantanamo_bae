...
import os
import time
from strategy import get_close_column  # Import function to detect correct Close column
...
def plot_trading_strategy(data, stock_symbol, buy_signals, sell_signals, output_dir="plots"):
    ...
    close_col = get_close_column(data)  # Detect correct Close column dynamically
    ...
    plt.plot(data.index, data[close_col], label='Close Price', color='steelblue')
